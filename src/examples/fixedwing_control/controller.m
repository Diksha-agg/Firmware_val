function [M, Thrust] = controller(posc, velc, rotc, omegac ,t)
coder.inline('never');
%desired inputs:[xd yd zd xd_dot yd_dot zd_dot phid thetad psid phidotd thetadotd psidotd ud(1)---Tfwd
%ud(2)---Mfwd
[posd, veld, rot_des, omegad, controld] = time_trajj(t);
Thrust=0;
BQ_m=12;%kg
BQ_g=9.81;
BQ_J=[1.86 0 0;
...
omega_des=[1 0 -sin(rot_des(2));0 cos(rot_des(3)) cos(rot_des(2))*sin(rot_des(3));0 -sin(rot_des(3)) cos(rot_des(2))*cos(rot_des(3))]*[omegad(3); omegad(2); omegad(1)];
...
c2=[-sin(rot_des(1)) cos(rot_des(1)) 0]';
